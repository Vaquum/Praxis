...
emits the `CommandAccepted → OrderSubmitIntent → OrderSubmitted →
FillReceived` event-spine sequence (i.e. the slice that comes
directly off the binsim+adapter wire), and the slippage path
(`query_order_book → estimate_slippage`) executes cleanly against
binsim's `/api/v3/depth` response.

The downstream events `TradeClosed` / `TradeOutcomeProduced` are
generated by ExecutionManager's own state-machine transitions and
are out of scope for this smoke — `test_execution_manager.py`
covers those against a mocked adapter where ExecutionManager logic
