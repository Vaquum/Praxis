...
class _FakeAdapter:
    def __init__(self, trade_ids: list[int]) -> None:
        ...
        self.start_times: list[object | None] = []
    ...
    async def query_trades(
        ...
        *,
        ...
        del account_id, symbol, end_time
...
@pytest.mark.asyncio
async def test_bootstrap_uses_start_time_when_no_cursor() -> None:
    adapter = _FakeAdapter([1, 2, 3])

    trades, complete = await paginate_my_trades(
        adapter, _ACCT, _SYMBOL, from_id=None, start_time=_TS, page_limit=10,
    )

    assert [int(t.venue_trade_id) for t in trades] == [1, 2, 3]
    assert complete is True
    assert adapter.calls == [None]
    assert adapter.start_times == [_TS]


@pytest.mark.asyncio
async def test_non_numeric_page_boundary_stops_incomplete() -> None:
    adapter = _NonNumericAdapter(page_size=2)
