...
| `TRADE_MODE` | Trading mode selector. `paper` routes the venue adapter and `MarketDataPoller` at `testnet.binance.vision` (REST) and `testnet.binance.vision/ws-api/v3` (WebSocket); `live` routes both at the production endpoints. Set explicitly per environment — there is no default and no separate URL or testnet env var |
