...
Two foundations and two of the subjects the brief names. Modification comes next; it is carried out as a cancel followed by a replacement, so it reads better once cancellation is written.
...
| What a fill is | Needed as a link target by several of the above; seed it as soon as one of them needs it rather than pre-emptively. |
...
| H-028 | A replacement that fails to place ends the whole order as cancelled, counting the fills from both the old order and the failed replacement. The original is already gone by then. | `_emit_amend_failed` 10481-10513 | P2-01 | folds into P2-01 |
| H-029 | A replacement that times out is not assumed dead. The venue is asked about it first, and if the venue has it the change carries on normally. | `_process_modify` 10355-10366 | P2-01 | folds into P2-01 |
| H-030 | If that question to the venue cannot be answered, the order is ended as cancelled anyway, though the venue may hold the replacement. Same gap as U-01, on the replacement. | 10360-10365 with `_rescue_by_client_order_id` 5505-5511 | P2-01 | folds into P2-01; extends U-01 |
| H-031 | Changing a bracket's protective prices is a cancel and replace, not an edit. The resting pair is cancelled, the exposure is recomputed from the venue, and a new pair is placed. | `_process_bracket_modify` 9366-9379 | P2-02 | folds into P2-02 |
| H-032 | Each durable record is written before the venue action it authorises, so a crash cannot leave an action nobody recorded. | 9370-9375 | P2-02 | folds into P2-02 |
| H-033 | Three endings: protection active again, an unclear cancel that stops and waits for reconciliation, or a definite failure to replace. What happens after a failure is decided elsewhere. | 9376-9379 | P2-02 | folds into P2-02 |
| H-034 | Cancelling a running sliced order stops further scheduling and cancels any child still working at the venue. | `_abort_scheme` 6525-6532 | P2-03 | folds into P2-03 |
| H-035 | One cancelled result is reported for the whole order, and only once every child has settled — at once if none were working, otherwise as each cancel confirms. | 6528-6531 | P2-03 | folds into P2-03 |
| H-036 | A ladder caught mid-change retires both generations and places no replacement rungs. | 6530-6532 | P2-03 | folds into P2-03 |
| H-037 | Changing slice count or interval re-plans only the slices not yet fired, and restarts the clock so the next one is a full new interval away. | `_process_scheme_modify` 10054-10062 | P2-04 | folds into P2-04 |
| H-038 | A new total at or below what has already fired is refused. Shrinking below what is done is not how you stop a scheme; cancelling is. | 10062-10065 | P2-04 | folds into P2-04 |
| H-039 | A successful change resumes a scheme frozen by a failed slice, but one frozen for protection reasons cannot be resumed this way and the change is refused. | 10065-10068, 10083-10087 | P2-04 | folds into P2-04 |
| H-040 | That a restart loses a schedule change is asserted by the docstring at 10068-10070 and by TD-135. Neither is a body. Re-trace the restart path before any article repeats it. | docstring only; body not traced | P2-04 | blocked: needs a body trace |
...
| H-041 | Changing the weight curve of a Scheduled VWAP order is not supported. | 10070-10071 | P2-04 | folds into P2-04 |
