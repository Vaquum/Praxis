...
6. loads venue filters for the union of:
   - active symbols (from replay state — symbols with open orders or positions), and
   - `bootstrap_filter_symbols` (configured at `Trading(...)` construction; the launcher passes `frozenset({_DEFAULT_SYMBOL})` so a fresh paper-trade boot primes filters before any sensor's first ENTER tick).

   `BinanceAdapter.load_filters` is idempotent: symbols already present in the cache are not refetched, so a multi-account boot that passes the same bootstrap set per account fetches each symbol's `exchangeInfo` exactly once.
