...
- **Boot-time open-order sweep**: at end of `_startup_account` (after `_reconcile_account`), call `query_open_orders(account_id)` per account (Binance `GET /api/v3/openOrders` accepts the symbol parameter as optional and returns the full account-wide list when omitted) — per-symbol sweeping is insufficient because managed symbols are derived from local state and would miss venue orders on untracked symbols (which is exactly the manual/shared-account class this TD blocks). For each returned venue open order whose `client_order_id` is not in `trading_state.orders`: log at WARNING level, include `venue_order_id` + `client_order_id` + `symbol` + `original_qty` + `executed_qty`. The current `VenueOrder` shape does not expose `time_in_force`; if `time_in_force` is wanted in the alert payload, the `VenueAdapter` / `VenueOrder` contract must be extended as part of the fix. Default behavior is alert-and-block (refuse to enter ready state); auto-cancel only behind explicit `--auto-cancel-orphan-venue-orders` flag.
- **Class B reconcile must verify before terminalizing**: `reconcile_orphan_commands` Class B path must call `query_order(account_id, symbol, client_order_id=client_order_id)` (account_id and symbol are reconstructable from the persisted `OrderSubmitIntent`) before emitting `_emit_orphan_rejection`. The recovery branch is selected on `executed_qty`, not on terminal/non-terminal status:
  - Live with zero fills (venue NEW / PENDING_NEW, `executed_qty == 0`): synthesize `OrderSubmitted` (using venue-returned `venue_order_id`) instead of REJECTED; let `_reconcile_account` / WS handle from there.
  - Live with partial fills (venue PARTIALLY_FILLED, `executed_qty > 0`): must NOT synthesize REJECTED — must reconstruct `OrderSubmitted` + `FillReceived` for the executed amount and emit a non-terminal PARTIAL `TradeOutcomeProduced`; the order remains live and `_reconcile_account` / WS handles subsequent fills and the eventual terminal status.
  - Terminal with fills (venue FILLED, terminal CANCELED-with-partial-fill, terminal EXPIRED-with-partial-fill): must NOT synthesize REJECTED — must reconstruct `OrderSubmitted` + `FillReceived` for the executed amount and emit the matching terminal `TradeOutcomeProduced` (FILLED / CANCELED / EXPIRED) to bring Praxis/Nexus into venue truth. If reconstruction is infeasible (e.g. fee/commission detail unrecoverable from `query_order` alone), halt startup with operator-required reconciliation error rather than under-account the executed fills.
  - Terminal with zero fills (venue NotFound, CANCELED-zero-fill, EXPIRED-zero-fill, REJECTED): existing REJECTED synthesis is correct.
