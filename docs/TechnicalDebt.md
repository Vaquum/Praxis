...
The shape also decides what crosses into Nexus. Because only the quotient survives, neither measure can be re-derived against a single fill increment, so the translator forwards each one as a command-to-date snapshot on every fill-bearing outcome rather than as a per-increment series (`praxis/outcome_translator.py`). A consumer must take the latest measurement rather than the latest outcome, and must not weight one by the `fill_size` of the outcome carrying it: the measure spans every fill so far and the size spans one increment. Recording the inputs would remove that constraint too.

## TD-159: The launcher's per-account outcome queue is unbounded and has no consumer-stall policy

**Origin**: inventory of unmerged work, September 2026 (supersedes an unmerged round-11 note that claimed every runtime queue was unbounded)
**Severity**: Major potential impact, likelihood unmeasured — a sustained excess of production over consumption can exhaust process memory, and nothing reports that it is happening
**Module**: `praxis/launcher.py` (`_route_translated`), `praxis/core/execution_manager.py` (`_AccountRuntime`)

Each configured instance gets a `queue.Queue()` with no `maxsize`. `_route_translated` takes the account's wiring under a lock, applies the Nexus position and capital mutation synchronously, and then enqueues each translated outcome with `put_nowait` — one Praxis outcome can produce several. Enqueueing therefore offers no capacity backpressure: the producer is told nothing about how far behind the consumer is, and there is no depth at which it would learn. Nothing in the launcher drains the queue; it is handed to the per-instance thread, and the only reader is Nexus, which takes at most one fresh outcome per tick through `PraxisInbound.receive_outcome`.

Growth needs production to outrun consumption, and the sharpest case is a consumer that stops dequeuing altogether: a tick that no longer reaches `receive_outcome` because dispatch itself is wedged — a blocked strategy callback, a hung action submission, a dead thread. Not every slow path has that effect. An outcome parked as unresolved, or queued behind an unresolved sibling, has already been dequeued, so Praxis memory is released and the accumulation moves into Nexus instead. Conflating the two was the mistake in the note this replaces, and it matters because only the first is bounded by anything Praxis can do. A consumer that merely runs slower than a burst also accumulates, which is a rate mismatch rather than a stall and is addressed below.

The command queue's bound does not help. `command_queue` is capped and rejects overflow fail-closed as `CommandQueueFullError`, but it bounds queued commands and their admission reservations, not the outcomes they go on to produce: one command yields an acknowledgement, several partials and a terminal, and a bounded intake says nothing about that stream.

The other runtime queues are recorded here so the next reader does not re-derive them, not because they are equivalent. `admission_queue` is restrained by each producer awaiting the future it enqueued, which bounds it by the number of concurrent in-flight admissions rather than by a capacity. `ws_event_queue` has no production caller at all — every call to `enqueue_ws_event` is in the test suite, and live execution reports reach the writer through the awaited `admit` path in `praxis/trading.py` instead — so it is an unbounded public method rather than a live backlog. `priority_queue` is genuinely unfenced: `submit_abort` and `submit_modify` are public entry points that enqueue directly, so its rate is set by whoever calls them and not by the writer. `dispatch_queue` is internally fed, but admission futures complete before dispatch runs, so that alone does not establish a small backlog.

The severity split in the note this replaces — low for paper, major for production — does not hold. A consumer that stops consuming exhausts memory on a paper host by the same mechanism as on a production one; what differs is how often that happens and what a lost process costs, not whether the backlog can grow. Nothing observed says the running host has a growing backlog, and one queue exists per configured instance, so a multi-strategy basket multiplies the exposure without changing its shape.

The cheapest close is to make the condition visible: queue depth and the age of the oldest undelivered outcome, as a signal rather than a log line. Today there is no way to tell a healthy queue from one that has been filling for an hour, which is why the impact above is stated as potential. Separately, draining more than one outcome per tick would address a consumer that is merely slower than a burst, which is a rate mismatch rather than a stall and which no capacity limit fixes.

Closing the unbounded case itself is a policy question rather than a `maxsize` argument. Blocking the producer is not automatically backpressure: `_route_translated` runs on the Praxis event-loop thread, so a blocking put stalls the writer that fills, cancels and protects live orders. Dropping is not as absolute a loss as it first appears — the spine already records every outcome durably, and the TD-052 boot replay re-delivers produced outcomes with no matching acknowledgement — which suggests the lossless answer is to reuse that durable record for delivery rather than to invent a second one. Note also that stopping new commands does not stop fills and terminals arriving for orders already working, so a bounded queue still needs somewhere for those to go.

Related, and deliberately not folded into the above: Nexus retains outcomes behind an unresolved command in `_unresolved_retries`, which is a second accumulation on the same path, in a different repository, with a different trigger, and it is not silent or indefinite — it warns on each park and drops the whole unresolved group once the retry delays are exhausted. Bounding the Praxis queue does not touch it, and end-to-end backlog monitoring has to cover both.
