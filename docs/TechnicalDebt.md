...
**Module**: [`praxis/core/execution_manager.py`](../praxis/core/execution_manager.py) — `_handle_command` calls [`estimate_slippage`](../praxis/core/estimate_slippage.py) at line 949 but only logs the result; no rejection path. The estimator computes `simulated_vwap` and `slippage_estimate_bps` (VWAP vs mid) and returns; the command proceeds to `venue_adapter.submit_order` regardless.
