...
**Module**: [`praxis/binsim/ledger.py`](../praxis/binsim/ledger.py) — `apply_fill` line 320 (`account.fills.append`), `apply_order` line 442 (`account.fills.extend` + `account.seen_client_order_ids.add`), `_account_to_dict` line 567 (`sorted(account.seen_client_order_ids)` on the event loop).
