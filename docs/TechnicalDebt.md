...
- **Class B reconcile must verify before terminalizing**: `reconcile_orphan_commands` Class B path must call `query_order(client_order_id)` before emitting `_emit_orphan_rejection`. The recovery branch is selected on `executed_qty`, not on terminal/non-terminal status:
  ...
  - Any state with `executed_qty > 0` (live PARTIALLY_FILLED, terminal FILLED, terminal CANCELED-with-partial-fill, terminal EXPIRED-with-partial-fill): must NOT synthesize REJECTED — must reconstruct `OrderSubmitted` + `FillReceived` for the executed amount + the appropriate terminal `TradeOutcomeProduced` (PARTIAL / FILLED / CANCELED / EXPIRED) to bring Praxis/Nexus into venue truth. If reconstruction is infeasible (e.g. fee/commission detail unrecoverable from `query_order` alone), halt startup with operator-required reconciliation error rather than under-account the executed fills.
