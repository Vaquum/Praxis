...
- Add [`Trading.set_on_trade_outcome(cb)`](praxis/trading.py) — installs the `on_trade_outcome` callback after `Trading()` construction, closing the chicken-and-egg gap where `TradingConfig.on_trade_outcome` cannot reference the not-yet-built `Trading` instance. Sync callbacks are auto-wrapped in an `async` adapter so the underlying `ExecutionManager` can `await` them uniformly. Raises `RuntimeError` if called after `start()` — the replay loop and in-flight order coroutines rely on a stable callback identity, so swapping it after start would race with outcome production (PT.4.1)
- Add [`ExecutionManager.set_on_trade_outcome(cb)`](praxis/core/execution_manager.py) — the underlying setter that `Trading.set_on_trade_outcome` forwards to. Same pre-`start()` guard
- Wire [`launcher.py`](praxis/launcher.py) `_start_trading` to call `self._trading.set_on_trade_outcome(self._trading.route_outcome)` immediately after `Trading()` construction and before `await trading.start()`. Without this, `ExecutionManager._on_trade_outcome` stayed `None`, outcomes produced by `ExecutionManager` were silently dropped, and the OutcomeLoop registered by Praxis #73 read from an empty queue forever (PT.4.2)
- Add 5 tests in [`test_trading.py`](tests/test_trading.py) covering setter happy path (sync and async callback), pre-`start()` guard / post-`start()` raise, end-to-end `register_outcome_queue` + `set_on_trade_outcome(route_outcome)` + driving a fake `FillReceived` through `ExecutionManager` and asserting the registered queue yields the `TradeOutcome` (PT.4.3)
- 831 tests passing (up from 826). Bumps `vaquum-praxis` 0.42.0 → 0.43.0
