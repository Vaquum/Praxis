...
- Add `test_slow_fetch_does_not_accumulate_drift_in_fetch_schedule` and `test_multi_interval_slow_fetch_collapses_missed_slots` in [`tests/test_market_data_poller.py`](tests/test_market_data_poller.py). The drift test pins the cumulative-no-drift contract for sub-2-interval overruns: with `interval=0.1` and `slow_delay=0.15`, fetch #4's offset from fetch #1 is `4 * interval = 0.40s` for the anchored loop versus `3 * interval + slow_delay = 0.45s` for the pre-fix sleep-after-fetch loop; threshold at `4 * interval + interval / 4 = 0.425s` distinguishes deterministically. The skip test pins the multi-interval case: with `slow_delay = 2.5 * interval`, fetch #3's start is at the next future scheduled slot (gap ≈ `slow_delay + interval / 2`), not back-to-back with the slow fetch's return (gap ≈ `slow_delay`). A no-skip implementation that does `n += 1` fails the skip test deterministically; a pre-fix sleep-after-fetch implementation fails the drift test deterministically (both verified locally by swapping the loop body back and re-running the tests before restoring)
