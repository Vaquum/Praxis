...
- Carry `cumulative_notional` through `ExecutionManager._emit_outcome` (immediate-fill path), `_emit_terminal_outcome` (venue terminal path), and `_emit_ws_outcome` (WS-driven path) — the immediate-fill path captures `total_notional` from the inline `sum(...)`, the venue terminal path forwards `order.cumulative_notional`, and the WS path scales it via `order.cumulative_notional * cmd.qty / order.filled_qty` when overfill clamping fires (defer overflow-clamp drift as TD-037) (FINAL-MAJOR-07)
- Bump `vaquum-nexus` pin from `f4eab11` (PR #49 merge) to the round-17 paper-trade-readiness branch HEAD `31d4fa4` so the pinned Nexus version exposes `RiskState.lock`, `CapitalController.lock_cm()`, and the `outcome_id`-aware WAL codec required by FINAL-MAJOR-02 / 05 / 07
