...
- Soften `_translate_enum` (and the five `translate_*` wrappers) in [`praxis/command_translator.py`](praxis/command_translator.py) to pass `None` through unchanged. Pre-patch `submit_command_with_translated_params` raised `TypeError: maker_preference must be MakerPreference or an enum with a string .value, got NoneType` for every Nexus `Action` whose strategy did not set `maker_preference` — observed on the deployment server at `2026-05-10T14:31:57Z` for all four `StubStrategySFD_4permut` sensors after the v0.58.0 image landed. The downstream Praxis [`validate_trade_command._validate_maker_preference`](praxis/core/validate_trade_command.py) already treats `None` as "anything except `MAKER_ONLY`" (the inequality returns true for `None` so the function short-circuits), and `stp_mode` is stored on `TradeCommand` without ever being read at the venue boundary, so `None` was the implicit pre-v0.58.0 contract for both fields. Post-v0.58.0 the strict translator raised the contract above the historical bar without any caller-side code preparing for it
- Update [`tests/test_command_translator.py`](tests/test_command_translator.py): drop `test_translate_rejects_none` (which asserted the over-strict pre-v0.58.1 behaviour); add five `test_translate_*_passes_none_through` cases (one per translator) that pin the new `None`-passthrough contract so a future refactor cannot silently re-introduce the deployment-server crash. Foreign-enum re-keying, `STPMode` semantic mapping, and the `TypeError` path for non-`None` non-enum inputs (`translate_order_side('BUY')`) remain unchanged
